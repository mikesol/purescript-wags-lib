...
makeCofreeFunctionOfTimeScoreMaybeNumber notes' = makeFullScene $ FullScene
  ...
  emitter = makeBufferPoolWithAnchor ((map <<< map) (filterMap (\{ offset, rest } -> rest # map \r -> { offset, rest: \st -> const { pitch: r, pw: makePiecewise (map (over _1 (add st)) (pwl 1.0)) }})) (makeScore { startsAt: 0.0, rest: (map <<< map) (uncurry { duration: _, rest: _ }) (identityToMoore (map (lcmap _.time) notes')) }))
